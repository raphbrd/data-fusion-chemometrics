...
def plot_concentrations_per_mixtures(gt_concentrations, hat_concentrations, col_inds, col_labels, color_gt="black", color_hat="tab:blue"):
    ...
    fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(4.54, 3.5), sharex=False, sharey=False, dpi=150)
    axes = axes.flatten()
    ...
    for idx, ax in enumerate(axes[:5]):
        ...
        ax.plot(x_ticks, y, marker="o", markersize=4, linewidth=1, label="CMTF estimate", color=color_hat)
        ax.grid(False)
        ax.tick_params(axis="both", labelsize=6, left=True, bottom=True, color="grey")
        ax.spines[["top", "right"]].set_visible(False)
        ax.set_xlabel("mixtures", fontsize=8)
        ax.set_ylabel(rf"$\hat A_{idx + 1}$ - {col_labels[idx]}", fontsize=8)
        ax.set_ylim(-0.1, 0.7)
    axes[-1].axis("off")
